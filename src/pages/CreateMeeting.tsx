...
function SortableAgendaItem({
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-start gap-2 ring-1 ring-border rounded-sm p-3 bg-background">
      <button type="button" className="mt-2 cursor-grab text-muted-foreground hover:text-foreground" {...attributes} {...listeners}>
        <GripVertical className="h-4 w-4" />
      </button>
      <div className="flex-1 space-y-2">
        <Input value={item.title} onChange={(e) => onUpdate(item.id, "title", e.target.value)} placeholder="Titel på dagsordenspunkt" className="text-sm" />
        <Textarea value={item.description} onChange={(e) => onUpdate(item.id, "description", e.target.value)} placeholder="Beskrivelse (valgfrit)" className="text-sm min-h-[60px]" />
      </div>
      <button type="button" onClick={() => onRemove(item.id)} className="mt-2 text-muted-foreground hover:text-destructive transition-colors">
...
const CreateMeeting = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!perms.kanOpretteMoeder) {
      toast.error("Du har ikke tilladelse til at oprette møder.");
      return;
    }
    if (!title.trim()) { toast.error("Mødets titel er påkrævet."); return; }
    if (!orgId) { toast.error("Organisationsdata mangler — prøv at genindlæse siden."); return; }
    if (!memberId) { toast.error("Brugerdata mangler — prøv at genindlæse siden."); return; }
    ...
    try {
      ...
      if (validItems.length > 0) {
        const { error: agendaError } = await supabase.from("agenda_items").insert(
          validItems.map((item, i) => ({ meeting_id: meeting.id, org_id: orgId, title: item.title.trim(), description: item.description.trim() || null, sort_order: i }))
        );
      ...
      if (selectedMembers.length > 0) {
        await supabase.from("approvals").insert(
          selectedMembers.map((mid) => ({ meeting_id: meeting.id, org_id: orgId, member_id: mid, approved_at: null, token: null, token_expires_at: null }))
        );
      }
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar mode="single" selected={date} onSelect={setDate} locale={da} initialFocus className={cn("p-3 pointer-events-auto")} />
              </PopoverContent>
            </Popover>
          </div>
          <div className="space-y-2">
            <Label htmlFor="time" className="text-xs">Tidspunkt</Label>
            <Input id="time" type="time" value={time} onChange={(e) => setTime(e.target.value)} />
          </div>
        </div>

        <div className="space-y-2">
          <Label htmlFor="location" className="text-xs">Sted / lokation <span className="text-muted-foreground">(valgfrit)</span></Label>
          <Input id="location" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="F.eks. Kulturhuset, Mødelokale 2" />
        </div>

        <div className="space-y-3">
          <Label className="text-xs">Dagsordenpunkter</Label>
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={agendaItems.map((a) => a.id)} strategy={verticalListSortingStrategy}>
              <div className="space-y-2">
                {agendaItems.map((item) => (
                  <SortableAgendaItem key={item.id} item={item} onUpdate={updateAgendaItem} onRemove={removeAgendaItem} />
                ))}
              </div>
            </SortableContext>
          </DndContext>
          ...
          <div className="space-y-3">
            ...
            <div className="ring-1 ring-border rounded-sm divide-y divide-border">
              {orgMembers.map((member) => (
                <label key={member.id} className="flex items-center gap-3 p-3 cursor-pointer hover:bg-muted/30 transition-colors">
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Button type="button" variant="outline" size="sm" onClick={() => navigate("/moeder")}>Annullér</Button>
        </div>
      </form>
    </div>
  );
